...
export function searchName(lolArchive, search) {
  return lolArchive.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase())
  );
}

export const calc = (filterCalc, data) =>
  Math.round((filterCalc.length * 100) / data.length);
